...
class AdamP(optimizer_v2.OptimizerV2):
    ...
    def _resource_apply_dense(self, grad, var, apply_state=None):
        ...
        wd_ratio = 1
        if len(var.shape) > 1:
            perturb, wd_ratio = self._projection(var, grad, perturb, coefficients['delta'], coefficients['wd_ratio'],
                                                 coefficients['epsilon'])

        # Weight decay
        if self.weight_decay > 0 and self._do_use_weight_decay(var.name):
            var = state_ops.assign(var, var * (1 - coefficients['lr'] * coefficients['weight_decay'] * wd_ratio),
    ...
    def _projection(self, var, grad, perturb, delta, wd_ratio, eps):
        ...
        perturb, wd = control_flow_ops.cond(pred=cosine_max < compare_val,
                                            ...
                                            false_fn=lambda: self.channel_false_fn(var, grad, perturb, delta, wd_ratio,
                                                                                   eps))
    ...
    def channel_true_fn(self, var, perturb, wd_ratio, eps):
        ...
        var_n = var / (array_ops.reshape(math_ops.euclidean_norm(self._channel_view(var), axis=-1),
                                         shape=expand_size) + eps)
        perturb -= var_n * array_ops.reshape(math_ops.reduce_sum(self._channel_view(var_n * perturb), axis=-1),
                                             shape=expand_size)
        wd = wd_ratio
    ...
    def layer_true_fn(self, var, perturb, wd_ratio, eps):
        expand_size = [-1] + [1] * (len(var.shape) - 1)
        var_n = var / (
                array_ops.reshape(math_ops.euclidean_norm(self._layer_view(var), axis=-1), shape=expand_size) + eps)
        perturb -= var_n * array_ops.reshape(math_ops.reduce_sum(self._layer_view(var_n * perturb), axis=-1),
                                             shape=expand_size)
        wd = wd_ratio
